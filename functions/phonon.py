...
from atomate2.forcefields.jobs import MACERelaxMaker, MACEStaticMaker, ForceFieldRelaxMaker, ForceFieldStaticMaker 
...
from functions.metric import calculate_rmse, calculate_mae, calculate_r2, phonon_rmse, phonon_rrmse, phonon_mae, phonon_shifted_rmse, mean_phonon_frequency
...
def calculate_phonon(model_path: str,
                     structure: Structure,
                     run_dir: str,
                     name: str,
    ...
    phonon = PhononMaker(name = name, 
                        bulk_relax_maker=ForceFieldRelaxMaker(calculator_kwargs=calculator_kwargs, force_field_name="MACE"), 
                        born_maker=None,
                        phonon_displacement_maker=ForceFieldStaticMaker(calculator_kwargs=calculator_kwargs, force_field_name="MACE"), 
                        static_energy_maker=ForceFieldStaticMaker(calculator_kwargs=calculator_kwargs, force_field_name="MACE"),
                        min_length=supercell_size,
                        displacement=displacement,
                        symprec=symprec).make(structure, supercell_matrix=supercell_matrix)
...
class Phonon_Properties:
    ...
    def _load_phonopy_object(self):
        if self.phonon_object is None:
            if self.force_sets is None:
                self.phonon_object = load(phonopy_yaml=self.phonopy_yaml, factor=VaspToTHz, is_nac=False, symprec=self.symprec)
            else:
                self.phonon_object = load(phonopy_yaml=self.phonopy_yaml, factor=VaspToTHz, is_nac=False, symprec=self.symprec,
    ...
    def _compute_kpath(self):
        ...
        if self.path or self.kpath is None:
            ...
            highsymmkpath = KPathSeek(structure = structure, 
    ...
    def _get_band_qpoints(self):
        ...
        if self.band_qpoints is None:
            ...
            self.band_qpoints = get_band_qpoints(band_paths=band_paths, 
                                                 npoints=101)
    ...
    def _get_label_and_connection(self):
        if self.labels_for_plot or self.connections is None:
            ...
            npoints=101
            self.bands, self.labels_for_plot, self.connections = get_band_qpoints_by_seekpath(primitive = structure,
                                                                                              npoints=npoints,
    ...
    def _run_bands_structure_dict(self, paths=None, labels=None):
        if self.bandstructure_dict is None:
            ...
            phonon.run_band_structure(paths=paths, labels=labels, is_band_connection=True)
    ...
    def _run_dos(self, mesh = None, symmetry=True):
        ...
        phonon.run_mesh(mesh=mesh, 
                        is_mesh_symmetry=symmetry, 
                        is_time_reversal=symmetry
                        )
    ...
    def _calculate_metrics(self, other):
        ...
        self.rmse = calculate_rmse(true_value=freq1, 
                                evaluated_value=freq2
                                )
        self.mae = calculate_mae(true_value=freq1, 
                                evaluated_value=freq2
                                )
        self.r2 = calculate_r2(true_value=freq1,
                            evaluated_value=freq2
                            )
        ...
        mesh1 = self.mesh_dict
        mesh2 = other.mesh_dict
        self.phonon_rmse = phonon_rmse(true_mesh_dict=mesh1,
                                    evaluated_mesh_dict=mesh2
                                    )
        self.phonon_mae = phonon_mae(true_mesh_dict=mesh1,
                                    evaluated_mesh_dict=mesh2
                                    )
        self.phonon_scaled_rmse = phonon_shifted_rmse(true_mesh_dict=mesh1,
                                                    evaluated_mesh_dict=mesh2
                                                    )
        self.phonon_rrmse = phonon_rrmse(true_mesh_dict=mesh1,
                                        evaluated_mesh_dict=mesh2
                                        )
    ...
    def get_metrics(self, other):
        ...
        return self.rmse, self.mae, self.r2, self.phonon_rmse, self.phonon_mae, self.phonon_scaled_rmse, self.phonon_rrmse
    ...
    def get_displacements(self, other):
        ...
        try:
            self.rms_displacement, self.max_displacement = StructureMatcher().get_rms_dist(self.structure, 
                                                                                       other.structure
                                                                                       )
        except:
