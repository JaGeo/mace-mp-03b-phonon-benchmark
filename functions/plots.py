...
class PhononPlotter:
    ...
    def _plot_phonon_bands(self, ax, distances, frequencies, color, linestyle, linewidth):
        ...
        for path in range(num_paths):
            for band in range(num_bands):
                ax.plot(distances[path], frequencies[path, :, band], 
                        color=color, 
                        linestyle=linestyle, 
                        linewidth=linewidth)
    ...
    def beautiful_phonon_plotter(self):
        ...
        for i in range(len(self.distances_set)):
            ...
            ax.plot([], [], color=self.colors[i], linestyle=self.linestyles[i], label=self.legend_labels[i])
        ...
        xticks = self._add_vertical_lines_and_commensurate_points(ax, 
                                                                 np.array(self.distances_set)[0], 
                                                                 default_comm_points)
